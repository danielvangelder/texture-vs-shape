...
def output_results_to_file(model_name, results, output_file_path, session = 1):
    ...
    data = [[model_name, session, i + 1, 0, prediction, actual, i + 1, img_url] for i, (prediction, actual, img_url) in
            enumerate(results)]
    df = pd.DataFrame(data, columns=['subj', 'session', 'trial', 'rt', 'object_response', 'category', 'condition',
                                     'imagename'])
